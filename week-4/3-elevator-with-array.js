...
const dispatch = ( floor, direction ) => {
  let numFloorsAway = 100
  let indexOfClosest = 0

  // then just find closest elevator

  for ( let i = 0; i < allElevators.length; i++ ) {
     if ( Math.abs(allElevators[i].currentFloor - floor) < numFloorsAway ) {
       numFloorsAway = Math.abs(allElevators[i].currentFloor - floor)
       indexOfClosest = i
     }
  }
